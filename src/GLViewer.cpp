...
//    }
    ...
    if(!bodies.body_list.empty()){
        auto tracked_body_data = bodies.body_list.front();
    
        for(auto &it: bodies.body_list){
            if(distance >=  it.position.x * it.position.x + it.position.z * it.position.z){
                distance = it.position.x * it.position.x + it.position.z * it.position.z;
        ...
        if (renderObject(tracked_body_data, bodies.is_tracked) && in_area) {
            ...
            if (tracked_body_data.keypoint.size() == 18)
                createSKPrimitive(tracked_body_data, BODY_18_BONES, skeletons, clr_id);
            else if (tracked_body_data.keypoint.size() == 34) {
                createSKPrimitive(tracked_body_data, BODY_34_BONES, skeletons, clr_id);
            } else {
                createSKPrimitive(tracked_body_data, BODY_BONES_FAST_RENDER, skeletons, clr_id);            
                // Draw rest of the body with kp only, and smaller (for high link density parts like hands)
                createSKPrimitivePtOnly(tracked_body_data, getIdx(BODY_38_PARTS::RIGHT_WRIST) + 1, tracked_body_data.keypoint.size() - 1, skeletons, clr_id);
            }
        }
    }
